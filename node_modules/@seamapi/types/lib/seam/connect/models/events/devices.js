...
const battery_level = z
    ...
    .min(0)
    .max(1)
    .describe('Number in the range 0 to 1.0 indicating the amount of battery in the affected device, as reported by the device.');
const device_battery_status = z
    .enum(['critical', 'low', 'good', 'full'])
    .describe('Battery status of the affected device, calculated from the numeric `battery_level` value.');
...
export const lock_method = z
    ...
    .describe('Method by which the affected lock device was locked or unlocked. When the method is `keycode`, the `access_code_id` indicates the access code that was used, if reported by the device.');
...
`);
export const device_third_party_integration_detected_event = device_event.extend({
  ...
  `);
export const device_third_party_integration_no_longer_detected_event = device_event.extend({
...
`);
export const device_error_subscription_required_resolved_event = device_event.extend({
...
export const lock_locked_event = device_event.extend({
    ...
    method: lock_method.describe('Method by which the affected lock device was locked. When the method is `keycode`, the `access_code_id` indicates the access code that was used, if reported by the device.'),
}).describe(`
  ---
  route_path: /locks
  ---
...
export const lock_unlocked_event = device_event.extend({
    ...
    access_code_id: z
        ...
        .uuid()
        .optional()
        .describe('ID of the access code that was used to unlock the affected device.'),
    action_attempt_id: z
        .string()
        .uuid()
        .optional()
        .describe('ID of the action attempt associated with the unlock action.'),
    method: lock_method.describe('Method by which the affected lock device was unlocked. When the method is `keycode`, the `access_code_id` indicates the [access code](https://docs.seam.co/latest/capability-guides/smart-locks/access-codes) that was used, if reported by the device.'),
}).describe(`
  ---
  route_path: /locks
  ---
...
export const thermostat_climate_preset_activated_event = device_event.extend({
    ...
    thermostat_schedule_id: z
        ...
        .uuid()
        .nullable()
        .describe('ID of the thermostat schedule that prompted the affected climate preset to be activated.'),
    climate_preset_key: z
        .string()
        .describe('Key of the climate preset that was activated.'),
    is_fallback_climate_preset: z
        .boolean()
        .describe('Indicates whether the climate preset that was activated is the fallback climate preset for the thermostat.'),
}).describe(`
  ---
...
export const thermostat_manually_adjusted_method = z
    .enum(['seam', 'external'])
    .describe('Method used to adjust the affected thermostat manually. `seam` indicates that the Seam API, Seam CLI, or Seam Console was used to adjust the thermostat.');
...
`);
export const temperature_threshold_no_longer_exceeded_event = device_event.extend({
...
export const temperature_reached_set_point_event = device_event.extend({
    ...
    desired_temperature_celsius: z
        .number()
        .optional()
        .describe("Desired temperature, in °C, defined by the affected thermostat's cooling or heating set point."),
    desired_temperature_fahrenheit: z
        .number()
        .optional()
        .describe("Desired temperature, in °F, defined by the affected thermostat's cooling or heating set point."),
}).describe(`
  ---
  route_path: /thermostats
  ---
